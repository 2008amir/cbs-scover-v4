...
function dinoSvg({ score, best, dinoY, obstacles, over }) {
    ...
    for (const ob of obstacles) {
        if (ob.type === 'bird') {
            items += `<text x="${ob.x}" y="${ground - 110}" font-size="52" font-family="sans-serif">🦅</text>`;
        } else {
            items += `<text x="${ob.x}" y="${ground}" font-size="56" font-family="sans-serif">🌵</text>`;
        }
    }
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
  <defs>
  ...
  <text x="90" y="${ground - dinoY}" font-size="66" font-family="sans-serif">${over ? '💥' : '🦖'}</text>
  ${items}
...
function parseNotes(text, octave) {
    const out = [];
    const tokens = String(text).toUpperCase().match(/[A-G](?:#|B)?[0-9]?|[WETYU]|-|\s+/g) || [];
