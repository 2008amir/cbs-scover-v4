...
// .piano play -> web-viewer preview card + a playable piano voice note,
// so the user sees the piano and hears it inside the WhatsApp chat itself.
const DEMO_MELODY = 'C E G C5 B G E C';

async function sendPianoOnly(EliteProTech, m, svg, octave = 3) {
    ...
    await EliteProTech.sendMessage(m.chat, {
        ...
        contextInfo: {
            externalAdReply: {
                ...
                body: 'Preview & play — tap the keys sound below',
                mediaType: 1,
                previewType: 0,
        ...
        await EliteProTech.sendMessage(m.chat, { text: PIANO_SITE }, { quoted: m });
    });

    // Playable inside the chat container: a real piano voice note.
    try {
        const notes = parseNotes(DEMO_MELODY, octave);
        const opus = await toVoiceNote(renderWav(notes));
        if (opus) {
            await EliteProTech.sendMessage(m.chat, {
                audio: opus, mimetype: 'audio/ogg; codecs=opus', ptt: true
            }, { quoted: m });
        }
    } catch { /* preview already sent */ }
    return true;
}


async function sendLive(EliteProTech, m, kind, svg) {
    if (kind !== 'dino') return sendPianoOnly(EliteProTech, m, svg, 3);
...
async function handlePiano(EliteProTech, m, { args, prefix, reply }) {
    ...
    if (LIVE_RE.test(lowered)) return sendPianoOnly(EliteProTech, m, pianoSvg(octave, 'LIVE'), octave);
