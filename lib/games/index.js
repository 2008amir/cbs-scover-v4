...
const PIANO_SITE = process.env.PIANO_SITE_URL || 'https://piano-kappa-orcin.vercel.app';

const PIANO_TEXT = '🎹 *CBS-SCOVER Piano*\n\nEnjoy the interactive piano directly from WhatsApp.\n\n🎵 Tap the button below to start playing.';

function pianoUsage(prefix) {
    return `🎹 *CBS-SCOVER Piano*\n\nUse:\n${prefix}piano play\n\nTap the button below to open the interactive piano.`;
}

/* Native-flow open_webview support check against the installed Baileys fork. */
function nativeFlowSupport() {
    try {
        const bail = require('baileys');
        const Interactive = bail?.proto?.Message?.InteractiveMessage;
        if (typeof bail.generateWAMessageFromContent !== 'function') return null;
        if (!Interactive?.NativeFlowMessage?.NativeFlowButton) return null;
        return bail;
    } catch { return null; }
}
...
async function sendPianoOnly(EliteProTech, m, svg) {
    ...
    try {
        await EliteProTech.sendMessage(m.chat, {
            ...
            contextInfo: {
                externalAdReply: {
                    title: '🎹 LIVE PIANO — CBS-SCOVER',
                    body: 'Tap to open and play: keys hover, press and sound',
                    mediaType: 1,
                    previewType: 0,
                    renderLargerThumbnail: true,
                    showAdAttribution: false,
                    thumbnail: png || undefined,
                    thumbnailUrl: png ? undefined : PIANO_SITE,
                    mediaUrl: PIANO_SITE,
                    sourceUrl: PIANO_SITE
                }
            }
        }, { quoted: m });
...
async function handleCommands(EliteProTech, m, ctx) {
    try {
        if (ctx.command === 'piano') return await handlePiano(EliteProTech, m, ctx);
        if (ctx.command === 'dino' || ctx.command === 'dinorun') return await handleDino(EliteProTech, m, ctx);
    } catch (err) {
        console.log('[games] command error:', err?.stack || err);
        await EliteProTech.sendMessage(m.chat, {
