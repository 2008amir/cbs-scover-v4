...
const isStandalone = import.meta.url === pathToFileURL(process.argv[1] || '').href

/* When V2 runs inside the V1 process they share one HTTP server, so only bind
   the port when V2 is started on its own. */
if (isStandalone) {
    server.listen(PORT, () => {
